...
mkdir -p release
tar -czf release/xw-mindmap-agent-skill-0.1.0.tgz -C agent-skills xw-mindmap install-xw-mindmap.sh
...
```bash
tar -tzf release/xw-mindmap-agent-skill-0.1.0.tgz
...
```bash
git tag xw-mindmap-skill-v0.1.0
git push xw xw-mindmap-skill-v0.1.0
```
...
```markdown
Initial xw-mindmap agent Skill release.
...
- Direct command mode is not included yet.
```
